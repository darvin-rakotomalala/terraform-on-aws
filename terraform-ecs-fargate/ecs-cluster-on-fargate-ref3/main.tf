...
module "autoscaling" {
  ...
  max_capacity           = 4
  min_capacity           = 1
  resource_id            = "service/${var.cluster_name}/${var.service_name}" # "service/${aws_ecs_cluster.main.name}/${aws_ecs_service.main.name}"
  ...
  target_value           = 80
  depends_on             = [module.ecs.my_service] // Explicit dependency
}
