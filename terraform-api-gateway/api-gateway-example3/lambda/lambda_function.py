...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Handle PATCH Request - Update a Book
            ...
            if (
                not body
                or "book_id" not in body
                or "update_key" not in body
                or "update_value" not in body
            ):
                return generate_response(
                    400, "Missing required fields: book_id, update_key, update_value"
                )

            return update_book(
                body["book_id"], body["update_key"], body["update_value"]
            )
...
def save_book(item):
    try:
        ...
        return generate_response(
            201, {"Message": "Book saved successfully", "Item": item}
        )
...
def delete_book(book_id):
    try:
        response = table.delete_item(Key={"book_id": book_id}, ReturnValues="ALL_OLD")
