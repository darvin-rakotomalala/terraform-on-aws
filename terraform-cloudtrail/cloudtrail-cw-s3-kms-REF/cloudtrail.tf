      ...
      "Statement": [
        {
          ...
          "Resource": [
            "arn:aws:logs:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:log-group:${aws_cloudwatch_log_group.cloudtrail.id}:log-stream:*"
          ]
        },
        {
...
resource "aws_cloudwatch_log_group" "cloudtrail" {
  name = "/aws/cloudtrail/my-cloudtrail-logs"
...
resource "aws_cloudtrail" "example" {
  ...
  depends_on = [
    ...
    aws_iam_role_policy.cloudtrail_example
  ]
...
resource "aws_cloudwatch_metric_alarm" "root_login" {
  alarm_name          = "root-access-${data.aws_region.current.region}"
...
resource "aws_cloudwatch_metric_alarm" "console_without_mfa" {
  alarm_name          = "console-without-mfa-${data.aws_region.current.region}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "action_without_mfa" {
  alarm_name          = "action-without-mfa-${data.aws_region.current.region}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "illegal_key_use" {
  alarm_name          = "key-changes-${data.aws_region.current.region}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "security_group_change" {
  alarm_name          = "security-group-changes-${data.aws_region.current.region}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "iam_change" {
  alarm_name          = "iam-changes-${data.aws_region.current.region}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "routetable_change" {
  alarm_name          = "route-table-changes-${data.aws_region.current.region}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
...
resource "aws_cloudwatch_metric_alarm" "nacl_change" {
  alarm_name          = "nacl-changes-${data.aws_region.current.region}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
